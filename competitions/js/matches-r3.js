/* matches-r2.js — cache-bust mirror of matches.js (identical content).
   GitHub Pages' Fastly CDN caches by PATH and ignores ?v=, so an edited
   matches.js can stay stale for a long time. nox-jun2026 loads THIS path
   to force a fresh fetch. Keep in sync with matches.js, or delete once a
   real cache-bust (content-hash filenames / working purge) is in place. */
